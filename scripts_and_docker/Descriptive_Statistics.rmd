...
## For testing
# output <- read_csv("../example/sample-tumor-til-survival/output.csv")
# testDf = as.data.frame(output[grep("prediction", output$slideID),1:8])
# testDf$slideID = gsub(pattern = "prediction-",
#                       replacement = "",
#                       x = testDf$slideID)
# testDf = dplyr::full_join(testDf, df, by = "slideID")

# sampOnly = as.data.frame(output[grep("prediction", output$slideID, invert = T),
#                                  c(1,9:12)])
# 
# testDf = dplyr::full_join(testDf,
#                           output[grep("prediction", output$slideID, invert = T),
#                                  c(1,9:12)], 
#                           by = "slideID")
...
if(anyNA(summary(fit.coxph)$conf.int)){
...
for(i in toFacet){
   ...
   if(anyNA(summary(fit.coxph)$conf.int)){
      message(paste0(" . . . Cox confidence interval too wide to plot for ",i, " skipping . . . "))
   } else{
