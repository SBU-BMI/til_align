...
droppable = c(grep("*_Canc_patch", colnames(df)), ## *_ regex used to be flexible with subtype specific patch metrics
              ...
              grep("*percent*", colnames(df)),
              grep("*overlap", colnames(df)),
              grep("*patch", colnames(df)),
              grep("slide_level_call", colnames(df)))

## For testing
# params = list(csvPath = "~/Desktop/tilalign/heathCheck/heathtestOuts/output.csv",
#               nVars = 2,
